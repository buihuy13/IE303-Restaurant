...
public interface ProductRepository extends JpaRepository<Products, String>, JpaSpecificationExecutor<Products> {
    ...
    @Query("SELECT p FROM Products p "
            ...
            + "WHERE p.id = :id")
    Optional<Products> findWithDetailsById(@Param("id") String id);
    ...
    @Query("SELECT p FROM Products p "
            ...
            + "LEFT JOIN FETCH ps.size "
            + "WHERE p.slug = :slug")
    Optional<Products> findWithDetailsBySlug(@Param("slug") String slug);
    ...
    List<Products> findWithDetailsByRestaurant(@Param("restaurant") Restaurants restaurant);
    ...
    @Query("SELECT DISTINCT p FROM Products p " + "LEFT JOIN FETCH p.restaurant r "
            + // Lấy luôn Nhà hàng
