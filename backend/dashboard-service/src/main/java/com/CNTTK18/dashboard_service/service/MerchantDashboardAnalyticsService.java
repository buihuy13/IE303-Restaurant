...
import com.CNTTK18.dashboard_service.security.CurrentUserService;
...
@Slf4j
public class MerchantDashboardAnalyticsService {
    ...
    public void assertMerchantOrAdminAccess(UUID restaurantId) {
        if (currentUserService.hasRole("ADMIN")) {
            return;
        }

        if (!currentUserService.hasRole("MERCHANT")) {
        ...
        try {
            merchantRestaurant = restaurantDashboardDataClient.getRestaurantByMerchantId(userId);
        } catch (Exception ex) {
            log.error("Cannot validate merchant ownership for user {}", userId, ex);
            throw new ForbiddenException("Access denied");
        }
